...
public final class ConcurrentDeduplicatingConsumer<T> implements Consumer<T> {
    private final EqualityComparator<T> equalityComparator;
    private final Consumer<T> delegate;

    private final AtomicReference<T> lastValue = new AtomicReference<>();

    public ConcurrentDeduplicatingConsumer(EqualityComparator<T> equalityComparator, Consumer<T> delegate) {
